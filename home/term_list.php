...
<!-- To allow only admin to access the content -->      
    <div class="box box-primary">
		...
		<label class="box-title">รายการ ห้วงเวลาการประเมิน</label>
        ...
        <div class="box-body">
				...
				<form id="form1" action="<?=$rootPage;?>_list.php" method="get" class="form" novalidate>
					<div class="col-md-6">
						<div class="form-group">
	                        <label for="search_word">บางส่วนของ ปีการประเมิน ครั้งที่การประเมิน เพื่อใช้ค้นหาข้อมูล</label>
							<input id="search_word" type="text" class="form-control" name="search_word" data-smk-msg="Require search word.."required>
	                    </div>	
	                    <!--form-group-->
					</div>
					<!--/.col-md-->
           ...
           <?php
				$sql = "
				SELECT hdr.`id`, hdr.`seqNo`, hdr.`year`, hdr.`term`, hdr.`isCurrent`, hdr.`statusId`
				...
				$sql .= "ORDER BY hdr.isCurrent DESC, hdr.seqNo ASC
						LIMIT $start, $rows 
				";		
                //$result = mysqli_query($link, $sql);
				$stmt = $pdo->prepare($sql);	
           ...
           ?> 
            ...
            <table class="table table-striped">
                <tr style="background-color: #ffcc99;">
					...
					<th>ครั้งที่ ห้วงเวลาการประเมิน</th>
                    <th>ห้วงปัจจุบัน</th>
                    <th>สถานะ</th>
                    <th>#</th>
                </tr>
                ...
                <tr>
                    ...
                    <td>
						 <?php
						 switch($row['isCurrent']){ 	
							case 1 :
								echo '<a class="btn btn-success" name="btnRowSetCurrent" data-id="'.$row['id'].'" >ใช่</a>';
								break;
							case 0 :
								echo '<a class="btn btn-default" name="btnRowSetCurrent" data-id="'.$row['id'].'" >ไม่ใช่</a>';
								break;
							default :	
...
$(document).ready(function() {
	...
	$('a[name=btnRowSetCurrent]').click(function(){
		var params = {
			action: 'setCurrent',
			id: $(this).attr('data-id')		
		};
